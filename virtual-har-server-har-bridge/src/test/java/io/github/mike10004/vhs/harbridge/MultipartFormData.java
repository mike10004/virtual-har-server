...
public class MultipartFormData {
    ...
    public interface FormDataParser {
        List<FormDataPart> decodeMultipartFormData(MediaType contentType, String boundary, byte[] data) throws MultipartFormDataParseException;
    }
    ...
    static class NanohttpdFormDataParser implements FormDataParser {
         ...
         */
        @Override
        public List<FormDataPart> decodeMultipartFormData(MediaType contentType, String boundary, byte[] data) throws MultipartFormDataParseException {
            ...
            try {
                for (int boundaryIdx = 0; boundaryIdx < boundaryIdxs.length - 1; boundaryIdx++) {
                    ...
                    if (mpline == null || !mpline.contains(boundary)) {
                        throw new MultipartFormDataParseException(BAD_REQUEST, "BAD REQUEST: Content type is multipart/form-data but chunk does not start with boundary.");
                    }
                    ...
                    if (partHeaderLength >= len - 4) {
                        throw new MultipartFormDataParseException(BAD_REQUEST, "Multipart header size exceeds MAX_HEADER_SIZE.");
                    }
                    ...
                    if (partContentType == null) {
                        partContentType = MediaType.OCTET_STREAM.toString();
                    }
                    // Read it into a file
                ...
                throw new MultipartFormDataParseException(INTERNAL_ERROR, e);
            }
            return parts;
        }
    }
    ...
    public static class MultipartFormDataParseException extends Exception {

        public final int status;

        public MultipartFormDataParseException(int status, String message) {
            super(message);
            this.status = status;
        }

        public MultipartFormDataParseException(int status, Throwable cause) {
            super(cause);
            this.status = status;
        }
    }
    ...
    private static class ByteSourceSlicingFormDataParser {
        static List<ByteSource> split(ByteSource whole, byte[] boundaryBytes, int minNumBoundaries) throws MultipartFormDataParseException {
            int[] boundaryIdxs;
            try {
        ...
        static List<ByteSource> split(ByteSource whole, int[] boundaryIdxs, int boundaryLength, int minNumBoundaries) throws MultipartFormDataParseException {
            if (boundaryIdxs.length < minNumBoundaries) {
                throw new MultipartFormDataParseException(BAD_REQUEST, "BAD REQUEST: Content type is multipart/form-data but contains less than two boundary strings.");
            }
