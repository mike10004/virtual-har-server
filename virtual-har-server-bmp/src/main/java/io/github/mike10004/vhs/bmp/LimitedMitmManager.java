/*
 * Mostly a copy of ImpersonatingMitmManager from net.lightbody.bmp.mitm.manager.
 * See that project for copyright info and license terms.
 */
 ...
 */
public class LimitedMitmManager implements MitmManager {
    private static final Logger log = LoggerFactory.getLogger(LimitedMitmManager.class);
    ...
    private final Supplier<SslContext> upstreamServerSslContext = Suppliers.memoize(new Supplier<SslContext>() {
        ...
        public SslContext get() {
            return createUpstreamServerSslContext(serverCipherSuites, trustConfig);
        }
    }::get);
     ...
     * @param trustManagerFactory the trust store that will be used to validate upstream servers' certificates, or null to accept all upstream server certificates
     * @return an SSLContext to connect to upstream servers with
     */
    public static SslContext createUpstreamServerSslContext(Collection<String> cipherSuites, TrustConfig trustConfig) {
        SslContextBuilder sslContextBuilder = SslContextBuilder.forClient();
        trustConfig.configure(sslContextBuilder);
     ...
     */
    private java.util.function.Supplier<CertificateAndKey> rootCertificate;
     ...
     */
    public LimitedMitmManager(CertificateAndKeySource rootCertificateSource,
        ...
        rootCertificate = Suppliers.memoize(new Supplier<CertificateAndKey>() {
            @Override
            public CertificateAndKey get() {
                return LimitedMitmManager.this.rootCertificateSource.load();
            }
        }::get);

        this.trustConfig = requireNonNull(trustConfig, "trustConfig cannot be null");
    ...
    public static class Builder {
        ...
        private TrustConfig trustConfig = TrustConfig.fromSource(TrustSource.defaultTrustSource());
         ...
         */
        public Builder trustSource(@Nullable TrustSource trustSource) {
            return trustSource(TrustConfig.fromSource(trustSource));
        }

        public Builder trustSource(TrustConfig trustConfig) {
        ...
        public LimitedMitmManager build() {
